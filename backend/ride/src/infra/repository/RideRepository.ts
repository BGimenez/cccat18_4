...
export class RideRepositoryDatabase implements RideRepository {
	...
	async getRideById(rideId: string): Promise<Ride> {
		...
		return new Ride(rideData.ride_id, rideData.passenger_id, parseFloat(rideData.from_lat), parseFloat(rideData.from_long), parseFloat(rideData.to_lat), parseFloat(rideData.to_long), rideData.status, rideData.date, rideData.driver_id, parseFloat(rideData.distance), parseFloat(rideData.fare));
	}
